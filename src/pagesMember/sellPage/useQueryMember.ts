...
export const useQueryMember = () => {
  ...
  // 查询会员免费次数
  const handleQueryMember = async (originalPrice: number) => {
    ...
    try {
      ...
      if (memberFreeCount.value > 0) {
        ...
        orderPrice.value = originalPrice // 无免费次数，恢复原价
  ...
  // 清除会员状态（切换为非会员时调用）
  const clearMemberState = (originalPrice: number) => {
    memberPhone.value = ''
    memberChecked.value = false
    memberFreeCount.value = 0
    orderPrice.value = originalPrice
  }
